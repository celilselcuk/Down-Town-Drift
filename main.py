...
# game map
game_map = pygame.image.load('game map.png')
...
blue_obs = pygame.image.load('blue obstacle.png')
red_obs = pygame.image.load('red obstacle.png')
...
def game_over():
    ...
    if highscore < score_checker:
        highscore = score_checker
    highscore_message = my_font.render(f'Highscore: {highscore}', False, (255, 255, 255))
            ...
            if game_screen:
                if event.key == pygame.K_UP:
                    if car_y > 0:
                        car_y -= 150
                    if car_y - 50 < 0:
                        image_border_sound = mixer.Sound('image border sound.wav')
